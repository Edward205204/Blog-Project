...
type RegisterFormProps = {
  ...
  updateField: <K extends keyof RegisterFormValues>(field: K, value: RegisterFormValues[K]) => void
}
  ...
  return (
    <form className={cn('flex flex-col gap-6', className)} {...props} action={formAction}>
      <FieldGroup>
        ...
        {state?.error ? (
          <Field>
            <FieldDescription className='text-center text-destructive'>{state.error}</FieldDescription>
          </Field>
        ) : (
          <Field>
        ...
        <Field>
          ...
          <FieldDescription className='text-center'>
            Đã có tài khoản?{' '}
            <a href={URL.LOGIN} className='underline underline-offset-4'>
              Đăng nhập
            </a>
          </FieldDescription>
        </Field>
